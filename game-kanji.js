// =============================================
// 1. DATA KANJI - DIAMBIL DARI kanji.js
// =============================================
// Data kanji diambil dari variabel global kanjiData
// yang sudah didefinisikan di file kanji.js
// Pastikan file kanji.js di-load TERLEBIH DAHULU
...
let questionTimeLimit = 15; // 15 detik karena lebih sulit
...
function handleTimeUp() {
    if (gameOver) return;
    playTimeUpSound();
    tampilkanNotifikasi(`Waktu habis! Jawaban: ${currentQuestion.hiragana} - ${currentQuestion.meaning}`, '⏰', 1500);
...
function nextQuestion() {
    ...
    // CEK APAKAH DATA KANJI TERSEDIA
    if (typeof kanjiData === 'undefined') {
        console.error('ERROR: Data kanjiData tidak ditemukan! Pastikan file kanji.js di-load terlebih dahulu.');
        if (characterContainer) {
    ...
    // Filter data yang memiliki hiragana (bukan kanji tunggal tanpa bacaan)
    const validData = kanjiData.filter(item => item.hiragana && item.hiragana !== '-');
    
    // Jika data valid kosong, gunakan semua data
    const dataSource = validData.length > 0 ? validData : kanjiData;

    // Ambil pertanyaan acak dari dataSource
    const randomIndex = Math.floor(Math.random() * dataSource.length);
    currentQuestion = dataSource[randomIndex];
    
    if (characterContainer) {
        characterContainer.textContent = currentQuestion.kanji;
        characterContainer.className = 'character-container';
    }

    // Format jawaban: "hiragana - arti"
    const correctAnswer = `${currentQuestion.hiragana} - ${currentQuestion.meaning}`;
    const options = [correctAnswer];

    while (options.length < 4) {
        const randomOption = dataSource[Math.floor(Math.random() * dataSource.length)];
        const optionText = `${randomOption.hiragana} - ${randomOption.meaning}`;
    ...
    if (optionsContainer) {
        ...
        options.forEach(option => {
            ...
            // Pisahkan hiragana dan arti untuk tampilan yang lebih rapi
            const parts = option.split(' - ');
            const hiragana = parts[0] || option;
            const meaning = parts[1] || '';
            
            button.innerHTML = `
                <span class="hiragana-text">${hiragana}</span>
...
function checkAnswer(selected, buttonElement) {
    ...
    if (isCorrect) {
        ...
        playWrongSound();
        if (resultContainer) {
            resultContainer.textContent = `❌ Salah! Jawaban: ${currentQuestion.hiragana} - ${currentQuestion.meaning}`;
        ...
        allButtons.forEach(btn => {
            if (btn.textContent === correctAnswer || btn.innerHTML.includes(currentQuestion.hiragana)) {
...
// Cek apakah data kanjiData tersedia
if (typeof kanjiData !== 'undefined') {
    console.log('🎮 Game Tebak Kanji siap dimainkan!');
    console.log(`📚 Total kanji: ${kanjiData.length}`);
} else {
    console.error('❌ ERROR: Data kanjiData tidak ditemukan!');
    console.log('📌 Pastikan file kanji.js di-load TERLEBIH DAHULU sebelum game-kanji.js');
}